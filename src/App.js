...
export default class App extends React.Component {
    ...
    fetchPage = (requestPage) => {
        ...
        const limit = this.state.pokemonPerPage;
        const url = 'https://pokeapi.co/api/v2/pokemon';
        fetch(`${url}?limit=${limit}&offset=${ (requestPage - 1) * 12}`)
            .then(response => response.json())
            .then(data => this.setState({pokemones: data.results}))
            .catch(error => {
                console.log(error);
    ...
    getImage = (index) => {
        ...
        //          0   + 1  + (( 2 - 1 ) * 10) = 11
        //          0   + 1  + (( 3 - 1 ) * 10) = 21
        let id = (index + 1) + ((this.state.currentPage - 1) * 12);
            ...
            <div className="pokedex-container">   
                {this
                    ...
                    .map((pokemon, index) => {
                        ...
                        let id = (index+1) + ((this.state.currentPage - 1) * 12);
                        return (<Card key={index + 1} id={id} name={pokemon.name} url={pokemon.url} img={pokemonImg} />)
                    })
}

                <div className="container"></div>
                <Pagination fetchPageFn={this.fetchPage}/>
                <div className="container-top"> <a className="btn btn-outline-success" href="#top" >top</a> </div>
            </div>  
        )
    }
}